...
class LarixServer(BaseHTTPRequestHandler):
    def render_html_page(self, results_html="", speed_metric="", query_val=""):
        # We split the long single-line template into multiple blocks to completely prevent clipping
...
<head>
    ...
    <style>
        ...
        .remove-saved:hover {{ text-decoration: underline; }}
        ...
        .save-btn {{ position: absolute; top: 20px; right: 20px; background: none; border: none; font-size: 22px; color: #ccc; cursor: pointer; padding: 0; outline: none; transition: color 0.2s ease; }}
        ...
        .pwa-banner {{ display: none; position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); background: #1e392a; color: white; padding: 12px 20px; border-radius: 8px; box-shadow: 0 4px 15px rgba(0,0,0,0.2); z-index: 2000; width: 90%; max-width: 400px; text-align: left; justify-content: space-between; align-items: center; box-sizing: border-box; }}
        .pwa-btn {{ background: #2e6f40; color: white; border: none; padding: 6px 14px; border-radius: 4px; font-weight: bold; cursor: pointer; font-size: 13px; margin-left: 10px; flex-shrink: 0; }}
        .pwa-text {{ font-size: 13px; line-height: 1.3; margin: 0; }}
       def render_html_bottom(self):
        return """<div id="aboutModal" class="modal-overlay" onclick="closeAboutModalOutside(event)">
        <div class="modal-box">
    ...
    def do_GET(self):
        ...
        if "query" in params:
            ...
            response_content = self.render_html_page(results_html, speed_metric, query_val=params["query"]) + self.render_html_bottom()
        else: response_content = self.render_html_page() + self.render_html_bottom()
