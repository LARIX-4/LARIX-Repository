from http.server import HTTPServer, BaseHTTPRequestHandlerimport urllib.parse, json, time, osDB_FILE = "larix_database.json"class LarixServer(BaseHTTPRequestHandler):
    def get_about_and_script(self):
        ...
        document.getElementById('pwaInstallBtn').addEventListener('click',async()=>{{if(deferredPrompt){{deferredPrompt.prompt();deferredPrompt=null;document.getElementById('pwaBanner').style.display='none'}}}});window.addEventListener('DOMContentLoaded',()=>{{renderHistory();renderSavedList()}});
    </script></body></html>"""
...
☰LARIX MenuFavorites & Saved VaultNo saved researches yet.Recent Search HistoryNo history recorded.Clear Application DataLARIXDevelopment of a Web-Based Literature Indexer and Review of Related Literature Repository in Guisguis National High SchoolAbout Us<input type="text" name="query" id="searchInput" placeholder="Enter keyword" value="""" + query_val + """ required> Search""" + speed_metric + " " + results_html + ""
...
if "query" in params:
user_query = params["query"][0].strip().lower(); start_time = time.perf_counter(); matched_items = []
...
speed_metric = f'LARIX Performance Metrics: Found {len(matched_items)} result(s) in {retrieval_speed:.6f} seconds.'
results_html = ""
if matched_items:
...
results_html += f"""☆{raw_title}Citation Reference: ({author_year})APA 7th Edition Citation:
{apa_citation}Abstract: {abstract_text}Ready-to-Use RRL Snippet:
"{snippet_text}"View Verified Source Link"""
else: results_html = "No results found related to your keyword. Please try another term."
response_content = self.render_html_page(results_html, speed_metric, query_val=params["query"][0].replace('"', '"').strip())
