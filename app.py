from http.server import HTTPServer, BaseHTTPRequestHandlerimport urllib.parse, json, time, osDB_FILE = "larix_database.json"class LarixServer(BaseHTTPRequestHandler):
    ...
    def render_html_page(self, results_html="", speed_metric="", query_val=""):
        q_val_clean = query_val.replace('"', '&quot;').strip()
...
{apa_citation}Abstract: {abs_raw}Ready-to-Use RRL Snippet:
"{snip_raw}"View Verified Source Link"""
else: results_html = "No results found related to your keyword. Please try another term."
response_content = self.render_html_page(results_html, speed_metric, query_val=params["query"][0])
